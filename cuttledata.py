...
>>>>>>> 130c1f5 (adding code for daniella)
    ...
    def is_image_out_of_focus(self, image_no, show_image=False):
        #Thomas Lapv version
        try:
            masks = self.load_masks(image_no)
            mantle = self.get_mantle_pattern(image_no)
            mantle_256 = (mantle * 255).astype(np.uint8)
            LapV_value = LAPV(mantle_256)
            print("LAPV value:" + str(LapV_value))
            return LapV_value
        except:
    ...
    def process_frame(self, func, ii):
        try:
            return func(ii)
        except Exception as e:
            print(f"Error processing frame {ii}: {e}")
            return None  # Return None or a custom error indicator

    def safe_process_frame(self, func, frame_number):
        try:
            return self.process_frame(func, frame_number)
        except Exception as e:
    ...
    def get_parallel_values_across_sessions(self, func, processes = 16):
        ...
        # Filter out None values or error indicators
        self.values = [result for result in results if result is not None]

        print(self.values)
        print("Type of values: " + str(type(self.values)))
